...
Once the transaction is confirmed you can check the funds using `getbalance`
command
...
will be used. For different operating systems, these are:
...
1. Builds the unbonding transaction and sends it to Babylon Genesis
2. Waits for the signatures from the Covenant Emulators
3. Sends the unbonding transaction to the Bitcoin chain

...
This guide walks you through deploying a smart contract on the Babylon Genesis blockchain. The example contract we'll deploy is a storage contract that allows saving data with Bitcoin timestamps and verifying the Bitcoin finalization status.
...
2. Create an account
3. Request test tokens with the wallet you just created above
